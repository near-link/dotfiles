...
// Catppuccin gradient
vec3 getCatppuccinGradient(float t) {
    t = fract(t);
    vec3 c1 = vec3(245., 194., 231.) / 255.; // Pink
    vec3 c2 = vec3(203., 166., 247.) / 255.; // Mauve
    vec3 c3 = vec3(137., 180., 250.) / 255.; // Blue
    vec3 c4 = vec3(148., 226., 213.) / 255.; // Teal
...
const float TRAIL_THICKNESS = 1.2;  // 1.0 = full cursor height, 0.0 = zero height, >1.0 = funky aah
const float TRAIL_THICKNESS_X = 1.1;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (lineLength > minDist && baseProgress < DURATION - 0.001) {
        ...
        // Generate a flowing Catppuccin gradient based on time and the position along the trail!
        float progress = iTime * 0.5 - fadeProgress * 0.8;
        vec3 catColor = getCatppuccinGradient(progress);
        vec4 trail = vec4(sRGBToLinear(catColor), TRAIL_COLOR.a);
